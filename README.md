...
Tracked patterns: `*.srm`, `*.sav`, `*.state*`, `*.mcr`, `*.ps2`, `*.gci`, `*.ppst`, `*.dsv`, `*.SaveRAM`, `*.sra`, `*.eep`, `*.fla`. Only save data is ever stored — **never ROMs or firmware** (those live in the separate `arcade/` project).
